...
async function test() {
  ...
  console.log('evText len:', evText.length);
  const match = evText.match(/cevent\s*=\s*(\[[^]*?\]);/);
  if (evText.length > 0) {
      try {
          const eventsStart = evText.indexOf('[');
          const eventsEnd = evText.lastIndexOf(']');
          const eventsJson = evText.substring(eventsStart, eventsEnd + 1);
          const events = JSON.parse(eventsJson);
          console.log('Total events parsed:', events.length);
          const domeEvents = events.filter((e: any) => e.PLACE_NAME && e.PLACE_NAME.includes('大巨蛋'));
          console.log('Dome:', domeEvents.length);
      } catch (e) {
          console.log('err:', e);
      }
  }
